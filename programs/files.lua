...
local fileButtons = {
    ...
    ["open"] = {
        ...
        ["edit"] = function(path, file)
            if not file then return end
            local editTab = multishell.launch({shell=shell,multishell=multishell,textutils=textutils,require=require},"edit", path..file)
            multishell.setTitle(editTab,"["..file.."]")
            multishell.setFocus(editTab)
        end,
        ["paint"] = function(path, file)
            if not file then return end
            local editTab = multishell.launch({shell=shell,multishell=multishell,textutils=textutils,require=require},"paint", path..file)
            multishell.setTitle(editTab,"["..file.."]")
            multishell.setFocus(editTab)
        end,
    },
